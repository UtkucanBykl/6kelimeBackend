...
from ..serializers import LikeListSerializer, PostDetailSerializer, PostListSerializer
...
class PostTestCase(APITestCase):
    ...
    def test_create_post_with_correct_data(self):
        ...
        post_data = {'content': '1 2 3 4 5 6', 'category': category.id}
    ...
    def test_get_user_posts(self):
        ...
        token = Token.objects.get(user=self.user)
        Post.objects.create(
            user=self.user, content="1 2 3 4 5 6", category=self.category
        )
        client.credentials(HTTP_AUTHORIZATION='Token ' + token.key)
    ...
    def test_get_category_posts(self):
        ...
        url = reverse_lazy(
            'core:post-category', kwargs={'cat_name': self.category.name}
        )
        response = client.get(url)
        serializer = PostListSerializer(
            Post.objects.filter(category=self.category), many=True
        )
        self.assertEquals(response.data, serializer.data)
    ...
    def test_get_post_order_by_likes(self):
        ...
        Post.objects.create(
            user=self.user, content="1 2 3 4 5 7", category=self.category
        )
        Like.objects.create(post=post, user=self.user)
        url = reverse_lazy('core:most-like')
        response = client.get(url)
        data = sorted(Post.objects.filter(publish=True), key=lambda x: x.like_count)[
            ::-1
        ]
    ...
    def test_delete_post_with_incorrect(self):
        ...
        client = APIClient()
        post = Post.objects.create(
            user=user, content="1 2 3 4 5 6", category=self.category
        )
        Post.objects.create(
            user=self.user, content="1 2 3 4 5 6", category=self.category
        )
    ...
    def test_get_user_unpublish_post(self):
        client = APIClient()
        Post.objects.create(
            user=self.user, content="1 2 3 4 5 6", category=self.category, publish=False
        )
        Post.objects.create(
            user=self.user, content="1 2 3 4 5 7", category=self.category, publish=True
        )
        ...
        response = client.get(url)
        serializer = PostListSerializer(
            Post.objects.filter(publish=False, user=self.user), many=True
        )
        self.assertEquals(response.data, serializer.data)
    ...
    def test_get_post_like(self):
        ...
        serializer = LikeListSerializer(
            Like.objects.filter(user=self.user, post=post), many=True
        )
        self.assertEquals(response.data, serializer.data)
    ...
    def test_search_post_with_content_query(self):
        ...
        serializer = PostListSerializer(
            Post.objects.actives().filter(content__icontains='1 2 3'), many=True
        )
    ...
    def test_put_update(self):
        ...
        client.credentials(HTTP_AUTHORIZATION='Token ' + token.key)
        url = reverse_lazy('core:post-update', kwargs={'slug': post.slug})
        data = {'content': '1 2 3 4 5 5', 'category': self.category.id}
